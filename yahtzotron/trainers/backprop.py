def train_backprop(model, num_epochs, league_size=100, eps=1e-2, objective="win"):
    ...
    for i in range(num_epochs):
        ...
        logger.warning(
            "Player {} won with a score of {}", winner, scores[winner].total_score()
        )
        ...
        for p in range(num_players):
            ...
            for n in (1, 2):
                for t in range(ruleset.num_rounds):
                    ...
                    turn_reward[0, chosen_action] = (
                        final_actions[p][t][1] + winning_bonus
                    )
                    self._nets[f"roll_{n}"].fit(
                        turn_input, turn_reward, epochs=10, verbose=0
                    )
            ...
            for n in (1, 2):
                for t in range(ruleset.num_rounds):
                    ...
                    turn_label = to_categorical(
                        chosen_action, num_classes=ruleset.num_categories
                    )[np.newaxis]
